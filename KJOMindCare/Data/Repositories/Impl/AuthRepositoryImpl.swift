...
class AuthRepositoryImpl: AuthRepository {
    ...
    func login(email: String, password: String) async throws -> User {
        ...
        let user = User(
            uid: firebaseUser.id, fullName: firebaseUser.fullName, email: firebaseUser.email,
            role: "user")
        return user
    }
    ...
    func getUserProfile(userId: String) async throws -> User {
        guard let user: User = try await firestoreService.get(from: "users", id: userId) else {
            throw NSError(
