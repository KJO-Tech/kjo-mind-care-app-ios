...
class CommentRepositoryImpl: CommentRepository {
    ...
    func getCommentsForBlog(blogId: String) -> AnyPublisher<[Comment], Error> {
        ...
        let currentUserId = authRepository.currentUser?.uid ?? ""
    ...
    func addComment(blogId: String, comment: Comment) async throws -> String {
        let docRef = firestore.collection("blogs").document(blogId).collection("comments")
            .document()
    ...
    func updateComment(blogId: String, commentId: String, content: String, currentUserId: String)
        async throws
    {
        ...
        guard commentDoc.exists else {
            throw NSError(
                domain: "CommentRepository", code: 404,
                userInfo: [NSLocalizedDescriptionKey: "Comment not found"])
        }

        let comment = try commentDoc.data(as: Comment.self)

        guard comment.author.uid == currentUserId else {
            throw NSError(
                domain: "CommentRepository", code: 403,
